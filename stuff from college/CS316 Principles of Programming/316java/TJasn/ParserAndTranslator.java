...
public final class ParserAndTranslator {
    ...
    /* ???????? *///done
        ...
        accept(LPAREN);
        //~
        int codeAddress = Instruction.getNextCodeAddress();
        expr7();
        //~modify
        accept(RPAREN);
        JUMPONFALSEinstr jInstr =
                new JUMPONFALSEinstr(Instruction.OPERAND_NOT_YET_KNOWN);


        statement();
        //~
        new JUMPinstr(codeAddress);
        //~modify
    ...
    /* ???????? *///done
        if (getCurrentToken() == CHARSTRING) {
            ...
            //~
            int startLoc = LexicalAnalyzer.getStartOfString();
            //~
            int endLoc = LexicalAnalyzer.getEndOfString();
            //~
            new WRITESTRINGinstr(startLoc, endLoc);
            //~ modify moved down 3
            nextToken();
    ...
    /* ???????? *///done
        ...
        while (getCurrentToken() == EQ || getCurrentToken() == NE) {
            Symbols operation = getCurrentToken();
            nextToken();
            expr4();
            //~
            if (operation == EQ) {
